...
var renderUsersPhotos = function () {
  getUsersPhotosData();
  for (var i = 0; i < usersPhotosData.length; i++) {
    var userPhoto = usersPhotoTemplate.cloneNode(true);

    userPhoto.querySelector('.picture__img').setAttribute('src', usersPhotosData[i].url);
    userPhoto.querySelector('.picture__likes').textContent = usersPhotosData[i].likes;
    userPhoto.querySelector('.picture__comments').textContent = usersPhotosData[i].comments.length;
